...
: Node("armor_detector", options), hik_camera_params_(this), cam_info_manager_(this)
{
  ...
  capture_thread = std::make_unique<std::thread>([this]() {
    RCLCPP_INFO(get_logger(), "capture loop thread running");
    captureLoop();
  });
...
void ArmorDetectorNode::captureLoop()
{
  while (rclcpp::ok()) {
    ...
    if (debug_enabled_) {
      debug_pubs_->img_raw.publish(
        *cv_bridge::CvImage(std_msgs::msg::Header(), "bgr8", raw_img).toImageMsg());
    }
    img_queue_.push(raw_img);
  }
}

void ArmorDetectorNode::detectOnce()
{
  cv::Mat raw_img;
  if (!img_queue_.waitAndPop(raw_img)) {
