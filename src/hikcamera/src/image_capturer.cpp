...
public:
  Impl(const CameraProfile &profile, const char *user_defined_name) {
    ...
    try {
      ...
      MV_CC_SetFloatValue(camera_handle_, "AcquisitionFrameRate", 165.0);
      MV_CC_SetBoolValue(camera_handle_, "AcquisitionFrameRateEnable", true);
      MV_CC_SetEnumValue(camera_handle_, "PixelFormat",
  ...
  void update_profile(const CameraProfile &profile) {
    ...
    // Invert image
    MV_CC_SetBoolValue(camera_handle_, "ReverseX", profile.invert_image);
    MV_CC_SetBoolValue(camera_handle_, "ReverseY", profile.invert_image);

    // Exposure
    MV_CC_SetEnumValue(camera_handle_, "ExposureAuto",
                       MV_EXPOSURE_AUTO_MODE_OFF);
    MV_CC_SetFloatValue(camera_handle_, "ExposureTime",
                        profile.exposure_time.count());

    // Gain
    MV_CC_SetFloatValue(camera_handle_, "Gain", profile.gain);

    RCLCPP_INFO(logger_, "Camera parameters updated at runtime.");
  }
...
void ImageCapturer::update_profile(const CameraProfile &profile) {
  impl_->update_profile(profile);
}
